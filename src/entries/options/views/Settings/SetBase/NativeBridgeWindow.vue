...
const extensionId = "com.ptplugins.pt-depiler"; // Tauri MVP：NativeBridge 禁用，占位
...
async function grantPermission() {
  ...
  try {
    const granted = false; // Tauri MVP：NativeBridge 禁用，无 nativeMessaging 权限概念
    if (granted) {
      await refreshStatus();
    } else {
...
async function revokePermission() {
  permissionLoading.value = true;
  try {
    // Tauri MVP：NativeBridge 禁用，无权限移除
    await refreshStatus();
  } catch (e: any) {
