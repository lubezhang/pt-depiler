...
async function getNextFlushUserInfoAt() {
  // Tauri 迁移：Rust scheduler 不暴露下次刷新时间查询，暂以配置间隔估算
  const { enabled, interval = 1 } = configStore.userInfo.autoReflush ?? {};
  nextFlushUserInfoAt.value = enabled ? Date.now() + interval * 60 * 60 * 1000 : 0;
}
